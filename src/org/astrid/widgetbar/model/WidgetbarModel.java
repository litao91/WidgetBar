...
/**
 * Maintain in-memory state of the Launcher. It is expected that there should be
 * only one WidgetbarModel object held in a static. Also Provide APIs for
 * updating the database state for the WidgetBar
 */
public class WidgetbarModel {
	...
	public void loadWidgetbarItems(boolean isLaunching, Widgetbar widgetbar) {
		if (isLaunching && isWidgetbarLoaded()) {
			widgetbar.onWidgetbarItemsLoded(mWidgetbarItems,
					mWidgetbarAppWidgets);
			return;
		}
		...
		mWidgetbarLoaderThread = new Thread(mWidgetbarItemsLoader,
				"Widgetbar Items Loader");
		mWidgetbarLoaderThread.start();
	}

	boolean isWidgetbarLoaded() {
		return mWidgetbarItems != null && mWidgetbarAppWidgets != null
				&& mWidgetbarItemsLoaded;
	}
	...
	private class WidgetbarItemsLoader implements Runnable {
		...
		public void run() {
			...
			final ContentResolver contentResolver = AppContext.getInstance()
			...
			final Cursor c = contentResolver.query(
					WidgetbarSettings.Favoriates.CONTENT_URI, null, null, null,
					null);
			try {
				final int idIndex = c
						.getColumnIndexOrThrow(WidgetbarSettings.Favoriates._ID);
				final int appWidgetIdIndex = c
						.getColumnIndexOrThrow(WidgetbarSettings.Favoriates.APPWIDGET_ID);
				final int sessionIndex = c
						.getColumnIndexOrThrow(WidgetbarSettings.Favoriates.SESSION);
				final int cellXIndex = c
						.getColumnIndexOrThrow(WidgetbarSettings.Favoriates.CELLX);
				final int cellYIndex = c
						.getColumnIndexOrThrow(WidgetbarSettings.Favoriates.CELLY);
				final int spanXIndex = c
						.getColumnIndexOrThrow(WidgetbarSettings.Favoriates.SPANX);
				final int spanYIndex = c
						.getColumnIndexOrThrow(WidgetbarSettings.Favoriates.SPANY);
			...
			if (!mStopped) {
				// Create a copy of the lists in case the workspace loader is
				// restarted
				// And the list are cleared before the UI can go through them
				final ArrayList<ItemInfo> uiWidgetbarItems = new ArrayList<ItemInfo>(
						widgetbarItems);
				final ArrayList<WidgetbarAppWidgetInfo> uiWidgetbarWidgets = new ArrayList<WidgetbarAppWidgetInfo>(
						widgetbarAppWidgets);
				widgetbar.onWidgetbarItemsLoded(uiWidgetbarItems,
						uiWidgetbarWidgets);
			}
			mRunning = false;
		}
	...
	public void unbind() {
		unbindAppWidgetHostViews(mWidgetbarAppWidgets);
	}
	...
	public static void addItemToDatabase(Context context, ItemInfo item,
			int session, int cellX, int cellY, boolean notify) {
		...
		Uri result = cr
				.insert(notify ? WidgetbarSettings.Favoriates.CONTENT_URI
						: WidgetbarSettings.Favoriates.CONTENT_URI_NO_NOTIFICATION,
						values);
		if (result != null) {
	...
	static void updateItemInDatabase(Context context, ItemInfo item) {
		...
		final ContentResolver cr = context.getContentResolver();
		item.onAddToDatabase(values);
		cr.update(WidgetbarSettings.Favoriates.getContentUri(item.id, false),
				values, null, null);
	}
	...
	static void deleteItemFromDatabase(Context context, ItemInfo item) {
		final ContentResolver cr = context.getContentResolver();
		cr.delete(WidgetbarSettings.Favoriates.getContentUri(item.id, false),
				null, null);
	}
