	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		DatabaseHelper(Context context) {
			...
			mAppWidgetHost = new AppWidgetHost(context,
					Widgetbar.APPWIDGET_HOST_ID);
		}

		/**
		 * Send notification that we've deleted the AppWidgetHost probably as
		 * part of the initial database creation. The receiver may want to
		 * re-call AppWidgetHost#startListening() to ensure callbacks are
		 * correctly set.
		 */
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE favorites (" + "_id INTEGER PRIMARY KEY,"
					+ "intent TEXT," + "session INTEGER," + "cellX INTEGER,"
					+ "cellY INTEGER," + "spanX INTEGER," + "spanY INTEGER,"
					+ "itemType INTEGER,"
					+ "appWidgetId INTEGER NOT NULL DEFAULT -1," + "uri TEXT"
					+ ");");
