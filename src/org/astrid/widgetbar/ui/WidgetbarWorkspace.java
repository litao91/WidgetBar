...
public class WidgetbarWorkspace extends ViewGroup implements DragScroller {
     ...
     * @param CellX X coord. of upper left corner expressed as a cell position
     * @param CellY Y coord. of upper left corner expressed as a cell position
     * @param cellHSpan Width in cells
     * @param cellVSpan Height in cells
     * @param rect Rectangle into which to put the result
     */
    public void cellToRect(int cellX, int cellY, int cellHSpan, int cellVSpan, RectF rect) {
        ((CellLayout)getChildAt(mCurrentSession)).cellToRect(cellX,  cellY, cellHSpan, cellVSpan, rect);
    }
    ...
    void setCurrentSession(int currentSession) {
        mCurrentSession = Math.max(0,  Math.min(currentSession, getChildCount()-1));
     ...
     * @param child The child to add in one of the workspace's sessions.
     ...
     * 
     * @param child The child to add in the session
     * @param session The session in which to add the child.
     * @param x The X position of the child in teh screen's grid
     * @param y The Y position of the child in the screen's grid
     * @param spanX The number of cells spanned horizontally by the child.
     * @param spanY The number of cells spanned vertically by the child
     */
   ...
   public CellLayout.CellInfo findAllVacantCells(boolean[] occupied) {
     ...
     * @param child The child to add in one of the workspace's sessions.
     * @param spanX The number of the cells spanned horizontally by the child
     * @param spanY the number of cells spanned vertically by the child.
     */
    ...
    */
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
     ...
     /*
     @Override
     protected boolean requestChildRectangleOnScreen(View child, Rect rectangle, boolean immediate) {
         int session = indexOfChild(child);
         if(session != mCurrentSession || !mScroller.isFinished()) {
             snapToSession(session);
             return true;
         }
         return false;
     }
     */
    	 ...
    	 case MotionEvent.ACTION_MOVE:
    		 if(mTouchState == TOUCH_STATE_SCROLLING) {
    			 ...
    			 if(deltaX < 0) {
    				 ...
    				 final int avaliableToScroll = getChildAt(getChildCount() -1).getRight() -
    						 getScrollX() - getWidth();
     ...
     void snapToSession(int whichSession) {
    	 ...
    	 if(focusedChild!=null && changingSessions && focusedChild == getChildAt(mCurrentSession)){
     ...
     public void scrollLeft() {
    	 if(mNextSession == INVALID_SESSION && mCurrentSession  > 0 && mScroller.isFinished()) {
    		 snapToSession(mCurrentSession - 1);
    	 }
     }
     public void scrollRight() {
    	 if(mNextSession == INVALID_SESSION && mCurrentSession < getChildCount() -1 &&
