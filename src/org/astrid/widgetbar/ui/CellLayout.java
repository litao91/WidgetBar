...
/**
 * The implementation of CellLayout The CellLayout is the basic layout of the
 * widget bar, it will align the widget to the Grid
 */
public class CellLayout extends ViewGroup {
	...
	/**
	 * The constructor basically get the parameter from the context (Normally
	 * defined in the XML files) and save the parameters to the local variables
	 */
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		child.setId(((getId() & 0xFF) << 16) | (cellParams.cellX & 0XFF) << 8
				| (cellParams.cellY & 0XFF));
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_DOWN) {
			...
			if (!found) {
				...
				// until a
				// caller invokes getTag() to retrieve the result. Find the
				// vacant
	...
	/**
	 * For every get tag, it will clear the current vacant cell list, and
	 * generate a new vacant cell list that is revelent to the current cell.
	 */
	@Override
	...
	/**
	 * Find vacantCell of different sizes recursively starting from the current
	 * cell, add them to the vacant list once found, basically
	 * findIntersectingVacantCell of current
	 */
	...
	public static final class CellInfo implements ContextMenu.ContextMenuInfo {
		 ...
		 * be instantiated when the user long press an empty cell.
		 * 
		 * VacantCell is basically a linked list storing a pool of vacant cell,
		...
		static final class VacantCell {
			...
			// 100 seems a reasonable compromise given the size of VacantCell
			// and
			...
			/**
			 * Function to acquire a Vacant Cell Instead of creating a new
			 * instance on every acquisition, we look up the pool.
			 */
		 ...
		 * 
		 * @param cellXY
		 *            The array that will contain the position of the vacant
		...
		/**
		 * The real implementation if findCellForSpan, iterate through the list
		 * to find a cell that big enough for span
		 */
